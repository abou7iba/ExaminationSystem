...
$sql_sel_teacher = "SELECT * FROM notifications WHERE noti_add_by = '$teacher_id_s' AND noti_id = '$noti_id_edit' ";
$res = mysqli_query($con, $sql_sel_teacher);
//  loop to get all data 
while($row = mysqli_fetch_assoc($res)){
$noti_id  = $row['noti_id'];
$noti_text = $row['noti_text'];
$noti_for_band = $row['noti_for_band'];
$noti_for_department = $row['noti_for_department'];
}
} 
?>
        <label for="">تعديل اشعار </label><br>
<label for="">الشعبة :</label><br>
<select name="department" id="" class="selection" >
    <option disabled>الشعبة : </option>
<?php
if($noti_for_department == 'نظم المعلومات الادارية'){
echo '<option  value="'. $noti_for_department .'">'. $noti_for_department . '</option>';
...
else if($noti_for_department == 'علوم الحاسب'){
echo '<option  value="'. $noti_for_department .'">'. $noti_for_department . '</option>';
echo '<option  value="نظم المعلومات الادارية">نظم المعلومات الادارية</option>';
echo '<option  value="ادارة">ادارة</option>';
echo '<option  value="محاسبة">محاسبة</option>';
}
else if($noti_for_department == 'ادارة'){
echo '<option  value="'. $noti_for_department .'">'. $noti_for_department . '</option>';
echo '<option  value="نظم المعلومات الادارية">نظم المعلومات الادارية</option>';
echo '<option  value="علوم الحاسب">علوم الحاسب</option>';
echo '<option  value="محاسبة">محاسبة</option>';
}
else if($noti_for_department == 'محاسبة'){
echo '<option  value="'. $noti_for_department .'">'. $noti_for_department . '</option>';
echo '<option  value="نظم المعلومات الادارية">نظم المعلومات الادارية</option>';
echo '<option  value="علوم الحاسب">علوم الحاسب</option>';
echo '<option  value="ادارة">ادارة</option>';
...
<br>
        ...
        <textarea name="description" id="" cols="20" rows="5" class="selection" ><?php echo $noti_text?></textarea>
        <br>

        <input type="submit" value="تعديل" name="editnotibtn">
        </center>
        <br>
    </div>

<?php
if(isset($_POST['editnotibtn'])){
...
$update = "UPDATE `notifications` SET `noti_text`='$description',`noti_for_band`='$band',`noti_for_department`='$department' WHERE noti_id = '$noti_id_edit'";
mysqli_query($con, $update);

header('LOCATION: editnoti.php?edit='.$noti_id_edit.'');
}
?>
</form>
